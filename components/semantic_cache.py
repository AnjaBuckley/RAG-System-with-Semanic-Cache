...
class SemanticCache:
    ...
    def __init__(self, similarity_threshold: float = 0.98, table_name: str = "cache_entries", embedding_client_type: str = "nomic_ai"):
    ...
    def get(self, query: str) -> Optional[Tuple[str, bool]]:
        ...
        if hasattr(result, 'data') and result.data:
            ...
            if years_in_query and years_in_cached and years_in_query != years_in_cached:
                logger.info(f"Cache MISS: Different years mentioned ({years_in_query} vs {years_in_cached})")
                return None, False

            # Check if one query mentions a year and the other doesn't
            if (years_in_query and not years_in_cached) or (not years_in_query and years_in_cached):
                logger.info(f"Cache MISS: Year mismatch (one query mentions a year, the other doesn't)")
                return None, False
